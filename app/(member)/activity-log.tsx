...
const ActivityLog: React.FC = () => {
  ...
  const loadCheckInHistory = useCallback(async () => {
    ...
    // FIX #4: Also block "pending" enrollment status
    if (
      !userData?.gymId ||
  ...
  // FIX #3: REMOVED the redundant useEffect that re-subscribed on month change
  // Month changes only need local filtering, not Firestore re-fetch
  ...
  // FIX #1: nextMonth now uses timestamp comparison instead of broken month/year independent check
  const nextMonth = () => {
    const next = new Date(
      currentMonth.getFullYear(),
  ...
  // FIX #2: Safe parser for "YYYY-MM-DD" strings — avoids UTC timezone shift bugs
  const parseDateParts = (dateStr: string): { year: number; month: number } => {
  ...
  // FIX #2: getUniqueDaysInMonth uses parseDateParts instead of new Date()
  ...
  // FIX #2: monthlyTotalDuration uses parseDateParts instead of new Date()
  const monthlyTotalDuration = dailySessions
    .filter((dailySession) => {
      const { year, month } = parseDateParts(dailySession.date);
      return (
        month === currentMonth.getMonth() &&
        year === currentMonth.getFullYear()
      );
  ...
  // FIX #4: Also block "pending" enrollment status from seeing activity
  const hasNoGym =
    !userData?.gymId ||
    userData?.enrollmentStatus === "none" ||
    userData?.enrollmentStatus === "rejected" ||
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.loginPromptContainer}>
          ...
          <Text style={styles.loginPromptText}>
            {userData?.enrollmentStatus === "pending"
              ? "Your enrollment is being reviewed. Activity tracking will be available once approved!"
              : "Join a gym to start tracking your activity and building your workout streak!"}
          </Text>
          <TouchableOpacity
            style={styles.loginButton}
            onPress={() =>
      ...
      >
        ...
        <View style={styles.recentSessions}>
          ...
          {dailySessions.length === 0 ? (
            ...
            dailySessions
              .filter((dailySession) => {
                // FIX #2: Use parseDateParts instead of new Date() for UTC-safe filtering
                const { year, month } = parseDateParts(dailySession.date);
                return (
                  month === currentMonth.getMonth() &&
                  year === currentMonth.getFullYear()
              ...
              .map((dailySession, index) => {
                // FIX #2: Parse date parts manually for display to avoid UTC shift
                const parts = dailySession.date.split("-");
